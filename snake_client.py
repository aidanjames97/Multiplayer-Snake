...
def main():
    ...
    try:
        ...
        server_der = client_socket.recv(1024)
        server_public_key = rsa.PublicKey.load_pkcs1(server_der, format="DER")

        client_der = public_Key.save_pkcs1(format="DER")
        client_socket.send(client_der)

        def enc(msg):
            if msg == "":
                return False
            else:
                return rsa.encrypt(msg.encode('ascii'), server_public_key)
        ...
        while not quitGame:
            ...
            if not sent:
                ...
                client_socket.send(enc("c:getd"))

            # getting updated data from server for next draw cycle and parsing it
            ...
            # done parsing info display to screen now
